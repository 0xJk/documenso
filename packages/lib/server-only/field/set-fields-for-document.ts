  ...
  const persistedFields = await prisma.$transaction(async (tx) => {
    ...
    const results = await Promise.all(
      linkedFields.map(async (field) => {
        ...
        // Skip DB write for unchanged existing fields.
        ...
        // Collect audit log for field update.
        if (field._persisted && changes.length > 0) {
          auditLogDataToCreate.push(
            createDocumentAuditLogData({
        ...
        // Collect audit log for field creation.
        if (!field._persisted) {
          auditLogDataToCreate.push(
            createDocumentAuditLogData({
    ...
    // Batch insert all audit logs in a single query.
